...
pub(crate) fn py_to_json(value: &Bound<'_, PyAny>) -> PyResult<Value> {
    ...
    if let Ok(a) = value.extract::<PyReadonlyArray1<f64>>() {
        return Ok(Value::Array(
            a.as_slice()?.iter().map(|x| serde_json::json!(x)).collect(),
        ));
    }
    Err(PyValueError::new_err(format!(
        "cannot convert {value:?} to a material param"
    )))
}
...
impl PyGroup {
    ...
    fn set_optimization_mask(&mut self, v: Vec<i32>) -> PyResult<()> {
        if v.len() != 7 {
            return Err(PyValueError::new_err(
                "optimization_mask must have 7 entries.",
            ));
        }
    ...
    fn from_state(state: &Bound<'_, PyDict>) -> PyResult<Self> {
        let v = py_to_json(state.as_any())?;
        Ok(Self {
            inner: Rc::new(RefCell::new(ver(Group::from_state(&v))?)),
        })
    }
    ...
    fn set_thickness_error_type(&mut self, v: i32) -> PyResult<()> {
        self.inner.borrow_mut().thickness_error_type =
            ver(navette::structure::ErrorType::try_from_i32(v))?;
        Ok(())
    }
    #[getter]
    ...
    fn set_inh_delta_error_type(&mut self, v: i32) -> PyResult<()> {
        self.inner.borrow_mut().inh_delta_error_type =
            ver(navette::structure::ErrorType::try_from_i32(v))?;
        Ok(())
    }
    #[getter]
    ...
    fn set_roughness_error_type(&mut self, v: i32) -> PyResult<()> {
        self.inner.borrow_mut().roughness_error_type =
            ver(navette::structure::ErrorType::try_from_i32(v))?;
        Ok(())
    }
    #[getter]
    ...
    fn set_interface_error_type(&mut self, v: i32) -> PyResult<()> {
        self.inner.borrow_mut().interface_error_type =
            ver(navette::structure::ErrorType::try_from_i32(v))?;
        Ok(())
    }
    #[getter]
    ...
    fn thickness_error(&self, value: f64, seed: Option<u64>) -> f64 {
        self.inner
            .borrow()
            .thickness_error(value, &mut rng_for(seed))
    }
    ...
    fn inh_delta_error(&self, value: f64, seed: Option<u64>) -> f64 {
        self.inner
            .borrow()
            .inh_delta_error(value, &mut rng_for(seed))
    }
    ...
    fn sr_roughness_error(&self, value: f64, seed: Option<u64>) -> f64 {
        self.inner
            .borrow()
            .sr_roughness_error(value, &mut rng_for(seed))
    }
    ...
    fn interface_error(&self, value: f64, seed: Option<u64>) -> f64 {
        self.inner
            .borrow()
            .interface_error(value, &mut rng_for(seed))
    }
...
impl PyDictProvider {
    ...
    fn new(
        ...
        let grid = wavelength
            .map(|w| w.as_slice().map(|s| s.to_vec()))
            .transpose()?;
    ...
    fn grid<'py>(&self, py: Python<'py>) -> Option<Bound<'py, PyArray1<f64>>> {
        self.inner
            .grid()
            .map(|g| PyArray1::from_vec(py, g.to_vec()))
    }
    ...
    fn export_entry(&self, py: Python<'_>, name: &str) -> PyResult<Py<PyAny>> {
        ...
        match found {
            ...
            Some((_, Entry::Spec(spec))) => {
                let v =
                    serde_json::to_value(spec).map_err(|e| PyValueError::new_err(e.to_string()))?;
                json_to_py(py, &v)
            }
        }
    }
...
pub struct PyWeaverProvider {
    inner: navette::structure::WeaverProvider<
        std::sync::Arc<navette::spectralweave::opticalweaver::OpticalWeaver>,
    >,
}
...
fn assert_provider_grid(grid: Option<Vec<f64>>, wavelengths: Vec<f64>, what: &str) -> PyResult<()> {
    ...
    ver(navette::structure::assert_provider_grid(
        &G(grid),
        &wavelengths,
        what,
    ))
...
fn check_schema_version(state: &Bound<'_, pyo3::types::PyDict>, what: &str) -> PyResult<()> {
...
fn program_to_dict(py: Python<'_>, prog: navette::config::LoadedProgram) -> PyResult<Py<PyDict>> {
    ...
    for (label, shared) in prog.structures {
        ...
        structures.set_item(
            ...
            PyStructure {
                inner: shared,
                materials: Some(mats.clone_ref(py)),
            },
        )?;
    }
...
fn load_program_file(
    ...
    let prog = ver(navette::config::load_program_file(
        path,
        &wl,
        prefix.as_deref(),
    ))?;
...
fn load_materials_section(
    ...
    let inner = ver(navette::config::load_materials(
        &v,
...
fn load_architect_section(
    ...
    let map: HashMap<String, SharedStructure> = structures
        ...
        .collect();
    let arch = ver(navette::config::load_architect_shared(
        &v,
        &map,
        prefix.as_deref(),
    ))?;
...
fn load_program(
    ...
    let wl = wavelengths.as_slice()?.to_vec();
    let prog = ver(navette::config::load_program_json_prefixed(
        text,
        &wl,
        prefix.as_deref(),
    ))?;
...
fn py_entry(value: &Bound<'_, PyAny>) -> PyResult<Entry> {
    ...
    Err(PyValueError::new_err(
        "provider values must be nk arrays or spec dicts",
    ))
}
...
fn snapshot_provider(
    ...
    for name in needed {
        ...
        if has_contains {
            let known: bool = obj
                .call_method("contains", (name,), None)?
                .extract()
                .unwrap_or(true);
            if !known {
                continue;
...
fn complex_array(value: &Bound<'_, PyAny>, name: &str) -> PyResult<Vec<Complex64>> {
    ...
    Err(PyValueError::new_err(format!(
        "material '{name}': expected a complex nk array"
    )))
}
...
fn resolve_inputs(snapshot: &DictProvider) -> PyResult<Vec<f64>> {
    ...
    match len {
        ...
        None => Err(PyValueError::new_err(
            "provider is empty; nothing to resolve.",
        )),
    }
}
...
impl PyStructure {
    ...
    fn validate(&self, py: Python<'_>) -> PyResult<Vec<String>> {
        ...
        let snapshot = match &self.materials {
            ...
            Some(m) => {
                ...
                let needed: Vec<String> =
                    borrowed.layers.iter().map(|l| l.material.clone()).collect();
                drop(borrowed);
        ...
        let p = snapshot
            .as_ref()
            .map(|s| s as &dyn navette::structure::MaterialProvider);
    ...
    fn solver_inputs(&self, py: Python<'_>) -> PyResult<PySolverArrays> {
        ...
        let (groups, seq) = {
            let borrowed = self.inner.borrow();
            let warnings =
                ver(borrowed.gate(&borrowed.validate(Some(&snapshot)), "Navette_Structure"))?;
            emit_warnings(py, "Navette_Structure", &warnings)?;
            let seq: Vec<(Layer, bool)> = borrowed
                .layers
                .iter()
                .cloned()
                .map(|l| (l, false))
                .collect();
            let groups: HashMap<String, Group> = borrowed
                .groups
                .iter()
    ...
    fn bake_materials(
        ...
        if wl.is_empty() {
            return Err(PyValueError::new_err(
                "bake_materials: wavelengths must be non-empty.",
            ));
        }
        let snapshot = match &self.materials {
            None => {
                return Err(PyValueError::new_err(
                    "bake_materials: no material provider set.",
                ))
            }
            Some(m) => {
                let borrowed = self.inner.borrow();
        ...
        let mapping = ver(self
            .inner
            .borrow_mut()
            .bake_materials(&wl, &snapshot, &mut target))?;
        Ok((
            mapping.into_iter().collect(),
            PyDictProvider { inner: target },
        ))
    }
    ...
    fn get_group_for_material(&self, material_name: &str) -> PyGroup {
        match self.inner.borrow().groups.get(material_name) {
            Some(g) => PyGroup {
                inner: Rc::new(RefCell::new(g.borrow().clone())),
            },
...
impl PyArchitect {
    ...
    fn solver_inputs(&self, py: Python<'_>) -> PyResult<PySolverArrays> {
        ...
        let merged = self.inner.merged_groups().map_err(|e| {
...
{e}"
    ...
    fn from_state(state: &Bound<'_, PyDict>, materials: Option<Py<PyAny>>) -> PyResult<Self> {
        let v = py_to_json(state.as_any())?;
        Ok(Self {
            inner: ver(Architect::from_state(&v))?,
            materials,
        })
    }
