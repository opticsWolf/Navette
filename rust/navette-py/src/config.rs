...
use navette::smatrix::synthesis::design_config::{GroupRow, LayerRow, MaterialDef, StructureCfg};
...
pub(crate) fn gate_document(request_json: &str) -> PyResult<(String, Option<String>, String)> {
    ...
    let (kind, name, payload) =
        navette::config::gate_document(&raw).map_err(PyValueError::new_err)?;
    let text = serde_json::to_string(&payload).map_err(|e| PyValueError::new_err(e.to_string()))?;
    Ok((kind, name, text))
}
