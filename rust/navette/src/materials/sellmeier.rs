...
pub fn sellmeier_domain_check(
  ...
  for (idx, b, c) in terms {
    ...
    if c > 0.0 {
      poles.push(format!("C{idx}={c} um^2 (resonance at {:.4} nm)", c.sqrt() * 1000.0));
    }
  }
  let at_pole = terms
    .iter()
...
mod tests {
  ...
  fn check(wl: &Array1<f64>) -> Result<(), String> {
    ...
    sellmeier_domain_check(wl.view(), &out, BK7[0], BK7[1], BK7[2], BK7[3], BK7[4], BK7[5])
  }
  ...
  fn the_domain_edge_is_below_the_first_pole_not_at_it() {
    ...
    let out = sellmeier_nk(array![50.0].view(), BK7[0], BK7[1], BK7[2], BK7[3], BK7[4], BK7[5]);
    assert!(out[0].re.is_finite() && out[0].re < 1.0, "n(50 nm) = {}", out[0].re);
  ...
  fn a_wavelength_inside_the_first_resonance_is_refused() {
    ...
    assert!(e.contains("77.4"), "the nearest resonance must be named: {e}");
  }
