...
use super::optics_core::{
    nevot_croce_factors, redheffer_product_complex_field_inner, w_function_inner,
};
    ...
    pub fn solve(&self, requested: u64) -> Result<Solution, String> {
        ...
        macro_rules! f64buf {
            ($name:ident, $cond:expr) => {
                let mut $name: Option<Vec<f64>> = if $cond {
                    Some(vec![0.0; total_points])
                } else {
                    None
                };
            };
      ...
      ($name:ident, $bit:expr) => {
        ...
        let omega: Vec<f64> = self
            .wavls
            .iter()
            .map(|&l| 2.0 * PI * C_NM_PER_FS / l)
            .collect();
        let disp = |phi: &Option<Vec<f64>>| -> Option<(Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>)> {
            phi.as_ref()
                .map(|p| dispersion_channel(p, &omega, num_angles, num_wavs))
        };
        let disp_r_s = if requested & REQ_DISP_R_S != 0 {
            disp(&b_phi_rs)
        } else {
            None
        };
        let disp_r_p = if requested & REQ_DISP_R_P != 0 {
            disp(&b_phi_rp)
        } else {
            None
        };
        let disp_t_s = if requested & REQ_DISP_T_S != 0 {
            disp(&b_phi_ts)
        } else {
            None
        };
        let disp_t_p = if requested & REQ_DISP_T_P != 0 {
            disp(&b_phi_tp)
        } else {
            None
        };
        ...
        if requested & REQ_PHI_RS != 0 {
            keep_f64!("phi_rs", b_phi_rs);
        }
        if requested & REQ_PHI_RP != 0 {
            keep_f64!("phi_rp", b_phi_rp);
        }
        if requested & REQ_PHI_TS != 0 {
            keep_f64!("phi_ts", b_phi_ts);
        }
        if requested & REQ_PHI_TP != 0 {
            keep_f64!("phi_tp", b_phi_tp);
        }
        if requested & REQ_PHI_RBS != 0 {
            keep_f64!("phi_rbs", b_phi_rbs);
        }
        if requested & REQ_PHI_RBP != 0 {
            keep_f64!("phi_rbp", b_phi_rbp);
        }
        if requested & REQ_PHI_TBS != 0 {
            keep_f64!("phi_tbs", b_phi_tbs);
        }
        ...
        if let Some(t) = disp_r_s {
            dispmaps.push(("R_s".to_string(), [t.0, t.1, t.2, t.3]));
        }
        if let Some(t) = disp_r_p {
            dispmaps.push(("R_p".to_string(), [t.0, t.1, t.2, t.3]));
        }
        if let Some(t) = disp_t_s {
            dispmaps.push(("T_s".to_string(), [t.0, t.1, t.2, t.3]));
        }
        if let Some(t) = disp_t_p {
            dispmaps.push(("T_p".to_string(), [t.0, t.1, t.2, t.3]));
        }
...
fn derive_range(states: &[OpticalState], mut sinks: Sinks<'_>) {
    ...
    for (k, s) in states.iter().enumerate() {
        ...
        put!(
            b_dop_r,
            k,
            ((s1r * s1r + s2r * s2r + s3r * s3r).sqrt() / (s0r + 1e-20)).min(1.0)
        );
        put!(
            b_dop_t,
            k,
            ((s1t * s1t + s2t * s2t + s3t * s3t).sqrt() / (s0t + 1e-20)).min(1.0)
        );
        ...
        put!(
            b_delta_r,
            k,
            if rs < RS_FLOOR { 0.0 } else { s3r.atan2(s2r) }
        );
        put!(
            b_psi_t,
            k,
        ...
        put!(
            b_delta_t,
            k,
            if ts < TS_FLOOR { 0.0 } else { s3t.atan2(s2t) }
        );
...
pub const DISP_SUFFIXES: [(&str, usize); 4] = [("GD", 0), ("GDD", 1), ("TOD", 2), ("FOD", 3)];
...
pub fn needle_gradient(
    ...
    if want_pmb || want_pmb_t || want_pmb_a || want_pmb_tb || want_pmb_rb || want_pmb_ab {
    ...
    let target_of =
        |d: Demand, k: usize| demands[d as usize].0.as_ref().map(|t| t[k]).unwrap_or(0.0);
    let weight_of =
        |d: Demand, k: usize| demands[d as usize].1.as_ref().map(|t| t[k]).unwrap_or(1.0);
    ...
    let any_nonzero =
        |v: &Option<Vec<f64>>| v.as_ref().is_some_and(|a| a.iter().any(|&x| x != 0.0));
    ...
    let inc = match (&incoherent_flags, want_any_pmb) {
        (_, false) => None,
        (None, true) => return Err(String::from("NREQ_P_MB requires incoherent_flags")),
        (Some(a), true) => {
            let v = a;
            if v.len() != nl {
    ...
    let mask = match &host_mask {
        Some(a) => {
            let v = a;
            if v.len() != nl {
                return Err(String::from("host_mask must have n_layers entries"));
            }
            Some(v.to_vec())
        }
        None => None,
    ...
    let mb_locs = match &inc {
        Some(flags) => Some(locate_hosts_multiblock(
            thicknesses,
            flags,
            z_grid,
            mask.as_deref(),
    ...
    let coh_locs: Vec<(usize, f64)> = if want_p
        || want_pt
        || want_pa
        || want_pphi
        || want_ptb
    ...
    impl PointOut {
        fn empty() -> Self {
            PointOut {
                ch: std::array::from_fn(|_| [None, None]),
            }
        }
        #[inline]
    ...
    let outs: Vec<PointOut> = (0..total_points)
        .into_par_iter()
        .map(|k| {
            let a = k / num_wavs;
            let w = k % num_wavs;
            ...
            let ns: Vec<Complex64> = (0..nl)
                .map(|l| {
                    Complex64::new(n_stack_cache[base + l * 2], n_stack_cache[base + l * 2 + 1])
                })
                .collect();
                ...
                for (pi, &on) in pol_on.iter().enumerate() {
                    ...
                    if want_pt {
                        let mut v = p_coherent_t_from_fields(
                            ...
                            pol,
                            np_c,
                            target_of(Demand::T, k),
                            weight_of(Demand::T, k),
                            thicknesses,
                            start_idx,
                            idx_end,
                            z_grid,
                    ...
                    if want_pa {
                        o.set(
                            Ch::Pa,
                            pi,
                            p_coherent_a_from_fields(
                                &fields,
                                nsin_fi,
                                lam,
                                pol,
                                np_c,
                                target_of(Demand::A, k),
                                weight_of(Demand::A, k),
                                thicknesses,
                                start_idx,
                                idx_end,
                                z_grid,
                    ...
                    if want_pphi {
                        o.set(
                            ...
                            p_coherent_phi_from_fields(
                                ...
                                np_c,
                                channel,
                                target_of(Demand::Phi, k),
                                weight_of(Demand::Phi, k),
                                thicknesses,
                                start_idx,
                                idx_end,
                                z_grid,
                    ...
                    if want_ptb {
                        o.set(
                            ...
                            p_coherent_tb_from_fields(
                                ...
                                pol,
                                np_c,
                                target_of(Demand::Tb, k),
                                weight_of(Demand::Tb, k),
                                thicknesses,
                                start_idx,
                                idx_end,
                                z_grid,
                    ...
                    if want_prb {
                        o.set(
                            ...
                            p_coherent_rb_from_fields(
                                ...
                                pol,
                                np_c,
                                target_of(Demand::Rb, k),
                                weight_of(Demand::Rb, k),
                                thicknesses,
                                start_idx,
                                idx_end,
                                z_grid,
                    ...
                    if want_pab {
                        o.set(
                            ...
                            p_coherent_ab_from_fields(
                                ...
                                pol,
                                np_c,
                                target_of(Demand::Ab, k),
                                weight_of(Demand::Ab, k),
                                thicknesses,
                                start_idx,
                                idx_end,
                                z_grid,
                    ...
                    if want_disp {
                        ...
                        if r2 > 1e-20 {
                            for (zi, &(j, xi)) in coh_locs.iter().enumerate() {
                                ...
                                let da =
                                    needle_slopes4_ddz(&fields, nsin_fi, j, xi, np_c, pol, lam)
                                        [channel];
    ...
    let disp_chain: Vec<Option<Vec<Vec<Vec<f64>>>>> = match max_order {
        None => vec![None, None],
        Some(mo) => {
            let omega: Vec<f64> = wavls
                .iter()
                .map(|&l| 2.0 * std::f64::consts::PI * C_NM_PER_FS / l)
                .collect();
            pol_on
                .iter()
                .enumerate()
                .map(|(pi, &on)| {
                    ...
                    let q0: Vec<Vec<f64>> = outs
                        .iter()
                        .map(|o| o.get(Ch::Q, pi).unwrap().clone())
                        .collect();
                    let mut chain = vec![q0.clone()];
                    for _ in 0..mo {
                        let prev = chain.last().unwrap();
                        chain.push(spectral_gradient_step(
        ...
        for (pi, &on) in pol_on.iter().enumerate() {
            ...
            for o in &outs {
                match o.get(ch, pi) {
                    ...
                    None => return Err(String::from("internal error: missing output buffer")),
                }
            }
...
impl Solver {
    ...
    pub fn find_eigenmodes(
        ...
        let (real_vals, imag_vals, flat) = self.landscape(
            real_range,
            imag_range,
            points_real,
            points_imag,
...
pub fn scan_box(
    ...
    let landscape: Vec<f64> = (0..points_imag * points_real)
        ...
        .map(|idx| {
            ...
            char_func(
                n_slice, &inv_n, d_slice, rt_slice, rv_slice, lam, n_eff, pol,
            )
        })
        .collect();
...
pub fn nelder_refine(
    ...
    let clamp = |p: [f64; 2]| -> [f64; 2] {
        [
            if p[0].is_finite() {
                p[0].clamp(-bound, bound)
            } else {
                0.0
            },
            if p[1].is_finite() {
                p[1].clamp(-bound, bound)
            } else {
                0.0
            },
        ]
    ...
    loop {
        ...
        let f_ref = char_func_xy(
            &reflected, n_slice, &inv_n, d_slice, rt_slice, rv_slice, lam, pol,
        );
        ...
        if f_ref < values[best] {
            ...
            let f_exp = char_func_xy(
                &expanded, n_slice, &inv_n, d_slice, rt_slice, rv_slice, lam, pol,
            );
            if f_exp < f_ref {
        ...
        iter += 1;
        let size = ((simplex[0][0] - simplex[1][0]).powi(2)
            + (simplex[0][1] - simplex[1][1]).powi(2))
        .sqrt()
            + ((simplex[1][0] - simplex[2][0]).powi(2) + (simplex[1][1] - simplex[2][1]).powi(2))
                .sqrt()
            + ((simplex[2][0] - simplex[0][0]).powi(2) + (simplex[2][1] - simplex[0][1]).powi(2))
                .sqrt();
...
pub fn field_prof(
    ...
    let interface_props = |i: usize| -> (Complex64, Complex64, Complex64, Complex64) {
        ...
        let den_safe = if den.norm() < 1e-100 {
    ...
    let mut s_left: Vec<(Complex64, Complex64, Complex64, Complex64)> =
    ...
    for i in 0..n_layers - 1 {
        ...
        sg = redheffer_product_complex_field_inner(
            sg.0, sg.1, sg.2, sg.3, iface.0, iface.1, iface.2, iface.3,
        );
        s_left.push(sg);
    }

    // S_right[i] = S‑matrix from substrate up to the right side of layer i (i from n_layers-2 down to 0)
    let mut s_right: Vec<Option<(Complex64, Complex64, Complex64, Complex64)>> =
        vec![None; n_layers];
    s_right[n_layers - 1] = Some((
        Complex64::new(0.0, 0.0),
        Complex64::new(1.0, 0.0),
        Complex64::new(1.0, 0.0),
        Complex64::new(0.0, 0.0),
    ...
    for i in (0..n_layers - 1).rev() {
        ...
        let iface = interface_props(i);
        sg = redheffer_product_complex_field_inner(
            iface.0, iface.1, iface.2, iface.3, sg.0, sg.1, sg.2, sg.3,
        );
        s_right[i] = Some(sg);
    }
...
mod tests {
    ...
    fn both_layouts(nw: usize) -> (Vec<Complex64>, Vec<f64>) {
        let n = |l: usize, w: usize| {
            Complex64::new(
                1.0 + l as f64 * 0.5 + w as f64 * 0.01,
                l as f64 * 0.001 * w as f64,
            )
        };
    ...
    fn view_masks_match_python() {
        use super::super::core_engine::{
            REQ_A_AVG, REQ_A_P, REQ_A_S, REQ_DELTA_R, REQ_DOP_R, REQ_PSI_R, REQ_R_AVG, REQ_RP,
            REQ_RS, REQ_S0_R, REQ_S1_R, REQ_S2_R, REQ_S3_R, REQ_TS,
        };
        assert_eq!(
            super::rt_request("s").unwrap(),
            REQ_R_AVG | REQ_T_AVG | REQ_RS | REQ_TS
        );
        assert_eq!(
    ...
    fn from_raw_broadcast_matches_explicit() {
        ...
        let a =
            super::Solver::from_raw(&wl, &[0.0], false, &per_layer, 2, None, None, None, None, 2)
                .unwrap();
        let full = vec![
            Complex64::new(1.0, 0.0),
            Complex64::new(1.0, 0.0),
    ...
    fn eigen_tools_standalone() {
        ...
        let (re, im, flat) =
            super::scan_box(&col, &th, &rt, &rv, 600.0, 0, 1.5, 2.0, 0.0, 0.05, 4, 3);
        assert_eq!(re.len(), 4);
        assert_eq!(im.len(), 3);
        ...
        let (r, i, v) =
            super::nelder_refine(&col, &th, &rt, &rv, 600.0, 0, (1.7, 0.01), 1e-3, 1e-9, 50);
    ...
    fn solve_arrays_contract() {
        ...
        let (sol, warns) = super::solve_arrays(
            &idx,
            &[0.0, 0.0],
            &[false, false],
            &[0, 0],
        ...
        let (_, warns) = super::solve_arrays(
            &idx,
            &[5.0, 0.0],
            &[false, false],
            &[0, 0],
    ...
    fn the_flat_constructor_solves_what_the_layer_major_one_solves() {
        ...
        .unwrap();
        let b = Solver::from_wav_major_flat(
            &wl,
            &[0.2],
            &flat,
            2,
    ...
    fn a_flat_cache_of_the_wrong_length_is_rejected() {
        ...
        let ok = Solver::from_wav_major_flat(
            &wl,
            &[0.0],
            &flat,
            2,
        ...
        assert!(ok.is_ok());
        let err = Solver::from_wav_major_flat(
            &wl,
            &[0.0],
            &flat[..flat.len() - 2],
            2,
    ...
    fn the_flat_constructor_refuses_what_new_refuses() {
        ...
        assert!(
            call(&[0.0], &[0.0, 0.0], 5).is_err(),
            "unknown coherence mode"
        );
        assert!(
            call(&[0.0], &[0.0], 2).is_err(),
            "one thickness for two layers"
        );
    }
    ...
    fn both_reciprocal_paths_give_the_same_cache() {
        ...
        for nw in [8usize, RECIP_PAR_THRESHOLD] {
            ...
            let s = Solver::from_wav_major_flat(
                &wl,
                &[0.0],
                &flat,
                2,
    ...
    fn derive_all(states: &[OpticalState], split: bool) -> AllChannels {
        ...
        (
            vec![
                b_rs, b_rp, b_ts, b_tp, b_ravg, b_tavg, b_as, b_ap, b_aavg, b_psi_r, b_psi_t,
                b_delta_r, b_delta_t, b_dop_r, b_dop_t, b_diatt_r, b_diatt_t, b_s0r, b_s1r, b_s2r,
                b_s3r, b_s0t, b_s1t, b_s2t, b_s3t, b_retard_r, b_retard_t, b_phi_rs, b_phi_rp,
                b_phi_ts, b_phi_tp, b_phi_rbs, b_phi_rbp, b_phi_tbs, b_phi_tbp,
            ],
            vec![
                b_rs_c, b_rp_c, b_ts_c, b_tp_c, b_rbs_c, b_rbp_c, b_tbs_c, b_tbp_c, b_cross_r,
                b_cross_t,
            ],
        )
    }
    ...
    fn validation_refuses() {
        ...
        let s = Solver::new(
            &wl,
            &[0.0],
            &[Complex64::new(1.0, 0.0), Complex64::new(1.5, 0.0)],
            2,
            &[0.0, 0.0],
            &[0, 0],
            &[0, 0],
