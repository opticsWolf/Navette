...
fn check_weight(weight: f64, label: &str) -> Result<(), String> {
    if !weight.is_finite() || weight < 0.0 {
        return Err(format!(
            "{label}: weight must be finite and >= 0 (got {weight})."
        ));
    }
    Ok(())
}
...
fn allclose(a: &[f64], b: &[f64]) -> bool {
    a.len() == b.len()
        && a.iter()
            .zip(b.iter())
            .all(|(x, y)| (x - y).abs() <= 1e-08 + 1e-05 * y.abs())
}
...
fn ingest_spectral(weaver: &TargetWeaver, t: &SpectralTarget) -> Result<(), String> {
    ...
    let kind = TargetKind::from_str(&t.kind)
        .ok_or_else(|| "Invalid kind (use 'e', 'a', 'b', 'r', or 'c')".to_string())?;
    let mode = if t.spectral == "PDts" || t.spectral == "PDtp" {
        "phase"
    } else {
        t.normalization_mode.as_str()
    };
    ...
    frame.set_data(
        key.clone(),
        SpectralData::from_arc(Arc::from(t.values.as_slice())),
        Some(&t.wavelengths),
    )?;
...
fn ingest_angular(weaver: &TargetWeaver, t: &AngularTarget) -> Result<(), String> {
...
pub fn check_color_demand(t: &ColorTargetJson) -> Result<ColorDemand, String> {
    ...
    if t.integral {
        return Err(
            "color: 'integral' is refused (a color demand already is integral).".to_string(),
        );
    }
    ...
    ColorDemand::new(
        ...
        t.weight,
        t.yi_cx
            .unwrap_or(crate::smatrix::synthesis::color_merit::E313_CX_D65_10),
        t.yi_cz
            .unwrap_or(crate::smatrix::synthesis::color_merit::E313_CZ_D65_10),
        t.wavelength_range,
    )
}
...
pub fn compile_merit_spec(set: &TargetSet) -> Result<MeritSpec, String> {
    ...
    for (ti, t) in set.angular.iter().enumerate() {
        ...
        for (uid, idxs) in &by_uid {
            ...
            sorted.sort_by(|a, b| entries[*a].angle.partial_cmp(&entries[*b].angle).unwrap());
    ...
    for (ei, ti) in jobs {
        ...
        if diff.is_some() && !t.2 {
            return Err(format!(
                "spectral={:?} is differential-phase: pass phase=True.",
                t.0
            ));
        }
...
mod tests {
    ...
    fn color_refusals_name_both_sides() {
        ...
        assert!(
            bad(&|t| t.illuminant = IllumJson::Name("F2".to_string()))
                .contains("unknown illuminant")
        );
        assert!(
            bad(&|t| t.observer = CmfJson::Name("1964_10deg".to_string()))
                .contains("unknown observer")
        );
        assert!(
            bad(&|t| t.reference = ReferenceJson::Valid(ColorReference::Scalar(1.0)))
    ...
    fn p2_quantities_compile_and_gate_pairs() {
        ...
        assert!(
            ok(
                "LCh",
                ReferenceJson::Valid(
                    crate::smatrix::synthesis::color_merit::ColorReference::Triple([
                        60.0, 20.0, 100.0
                    ])
                ),
                "DeltaE2000"
        ...
        assert!(
            ok(
                "Y",
                ReferenceJson::Valid(
                    crate::smatrix::synthesis::color_merit::ColorReference::Scalar(0.5)
                ),
                "Channels"
            )
            .is_ok()
        ...
        assert!(
            ok(
                "Oklab",
                ReferenceJson::Valid(
                    crate::smatrix::synthesis::color_merit::ColorReference::Triple([0.5, 0.0, 0.0])
                ),
                "DeltaE76"
            )
            .unwrap_err()
    ...
    fn wavelength_range_forwards_and_misshaped_is_loud() {
        ...
        assert_eq!(spec.color_demands()[0].white, full.color_demands()[0].white);
        // Reversed range refuses with a named message.
        let mut set = color_set();
        set.color[0].wavelength_range = Some([550.0, 520.0]);
        assert!(compile_merit_spec(&set).unwrap_err().contains("lo < hi"));
    ...
    fn color_json_shapes_refuse_loud() {
        ...
        assert!(
            compile_merit_spec(&set4)
                .unwrap_err()
                .contains("[3] triple")
        );
        // Unknown top-level keys still denied.
        assert!(
            serde_json::from_value::<TargetSet>(serde_json::json!({"color": [], "bogus": 1}))
                .is_err()
        );
    }
