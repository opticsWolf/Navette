...
use crate::color::common::{REF_WHITE_D65, xyz_to_lab};
...
fn check_grid(name: &str, wl: &[f64], cols: usize) -> Result<(), String> {
    ...
    if wl.windows(2).any(|w| !(w[0] < w[1])) {
        return Err(format!(
            "color: {name} wavelengths not strictly increasing."
        ));
    }
    ...
    pub(crate) fn new(
        ...
        match (&quantity, &reference) {
            ...
            (_, ColorReference::Triple(_))
                if !matches!(
                    ...
                    ColorQuantity::DomWl
                        | ColorQuantity::White
                        | ColorQuantity::Y
                        | ColorQuantity::Yellow
                ) => {}
            (ColorQuantity::DomWl, _) => {
                return Err(
                    "color: quantity 'DomWl' needs a [2] [wavelength_nm, purity] reference."
                        .to_string(),
                );
            }
            (ColorQuantity::Y, _) => {
            ...
            (ColorQuantity::White, _) => {
                return Err(
                    "color: quantity 'White' needs a [2] [whiteness, tint] reference.".to_string(),
                );
            }
        ...
        match (quantity, distance) {
            (ColorQuantity::XyY, ColorDistance::DeltaE2000)
            | (ColorQuantity::XyY, ColorDistance::DeltaE76) => {
                return Err(format!(
                    "color: quantity 'XyY' with distance '{distance:?}' refused (DeltaE is Lab-space; use Channels)."
                ));
            }
            (ColorQuantity::Oklab, ColorDistance::DeltaE2000)
            | (ColorQuantity::Oklab, ColorDistance::DeltaE76) => {
                return Err(format!(
                    "color: quantity 'Oklab' with distance '{distance:?}' refused (use equal-tol Channels)."
                ));
            }
            (ColorQuantity::Y, ColorDistance::DeltaE2000)
            | (ColorQuantity::Y, ColorDistance::DeltaE76) => {
                return Err(format!(
            ...
            (ColorQuantity::DomWl, ColorDistance::DeltaE2000)
            | (ColorQuantity::DomWl, ColorDistance::DeltaE76) => {
                return Err(format!(
        ...
        if !weight.is_finite() || weight < 0.0 {
            return Err(format!(
                "color: weight must be finite and >= 0, got {weight}."
            ));
        }
...
pub(crate) fn color_of_xyz(
    ...
    match quantity {
        ...
        ColorQuantity::White | ColorQuantity::Yellow => Err(
            "color: quantity 'White'|'Yellow' is a pair/scalar index (no triple form).".to_string(),
        ),
    }
}
...
pub(crate) fn eval_color_covered(
    ...
    for j in 0..3 {
        ...
        if !(fp.is_finite() && fm.is_finite()) {
            return Err(
                "color: non-finite objective under FD step (singular map point).".to_string(),
            );
        }
        df[j] = (fp - fm) / (2.0 * h);
    }
    ...
    for (j, &sim_idx) in ws.idx.iter().enumerate() {
        ...
        let g =
            s * (df[0] * ws.cmf_res[j][0] + df[1] * ws.cmf_res[j][1] + df[2] * ws.cmf_res[j][2]);
        grad.push((sim_idx, g));
    }
    Ok((f0.sqrt(), grad))
}
...
mod tests {
    ...
    fn white_identity_y_exactly_one() {
        ...
        let (cmf_wl, cmf, illum_wl, illuminant) = toy();
        let xyz =
            xyz_of_spectrum(&ones, &cmf_wl, &cmf, &cmf_wl, &illuminant, &illum_wl, None).unwrap();
    ...
    fn ramp_matches_independent_trapezoid() {
        ...
        for c in 0..3 {
            let rect: f64 = (0..8)
                .rev()
                .map(|i| ramp[i] * illum[i] * cmf[i][c] * h)
                .sum();
    ...
    fn quantity_wrappers_are_bitwise() {
        ...
        assert_eq!(
            color_of_xyz(&xyz, &d.white, ColorQuantity::Lab).unwrap(),
            lab[0]
        );
        let mut xyy = [[0.0; 3]];
        xyz_to_xyy(&[xyz], &mut xyy);
        assert_eq!(
            color_of_xyz(&xyz, &d.white, ColorQuantity::XyY).unwrap(),
            xyy[0]
        );
    }
    ...
    fn refusals_name_the_culprit() {
        ...
        assert!(
            mk(
                ColorQuantity::XyY,
                ColorReference::Triple([0.3, 0.3, 0.5]),
                ColorDistance::DeltaE2000
            )
            .unwrap_err()
            .contains("XyY")
    ...
    fn lch_wrappers_are_bitwise() {
        ...
        assert_eq!(
            color_of_xyz(&xyz, &d.white, ColorQuantity::LCh).unwrap(),
            lch[0]
        );
    ...
    fn oklab_wrapper_is_bitwise_and_white_is_unit() {
        ...
        let d = p2_demand(
            ColorQuantity::Oklab,
            ColorReference::Triple([0.5, 0.01, -0.02]),
        );
        let xyz = [0.3, 0.4, 0.2];
        let mut adapted = [[0.0; 3]];
        ...
        assert_eq!(
            color_of_xyz(&xyz, &d.white, ColorQuantity::Oklab).unwrap(),
            ok[0]
        );
    ...
    fn y_scalar_is_exact_and_hue_wraps() {
        ...
        let dh = p2_demand(
            ColorQuantity::LCh,
            ColorReference::Triple([60.0, 20.0, -179.0]),
        );
    ...
    fn achromatic_gradients_stay_finite() {
        ...
        for (q, r) in [
            (
                ColorQuantity::LCh,
                ColorReference::Triple([50.0, 5.0, 10.0]),
            ),
            (
                ColorQuantity::Oklab,
                ColorReference::Triple([0.5, 0.0, 0.0]),
            ),
    ...
    fn domwl_spectral_complementary_and_white() {
        ...
        let gauss: Vec<f64> = wl
            .iter()
            .map(|w| (-((w - 550.0) / 15.0).powi(2)).exp())
            .collect();
        let xyz = xyz_of_spectrum(
            &gauss,
            wl,
        ...
        let mag: Vec<f64> = wl
            .iter()
            .map(|w| (-((w - 450.0) / 12.0).powi(2)).exp() + (-((w - 650.0) / 12.0).powi(2)).exp())
            .collect();
        let xyz_m = xyz_of_spectrum(
            &mag,
            wl,
            &d.cmf,
            &d.cmf_wl,
    ...
    fn domwl_shape_and_pair_gates() {
        ...
        assert!(
            mk(ColorQuantity::DomWl, ColorReference::Triple([0.0; 3]))
                .unwrap_err()
                .contains("DomWl")
        );
        assert!(
            mk(ColorQuantity::Lab, ColorReference::Pair([550.0, 0.9]))
                .unwrap_err()
                .contains("pair reference")
        );
    ...
    fn coordinate_quantities_are_bitwise_and_smooth() {
        ...
        assert_eq!(
            color_of_xyz(&xyz, &white, ColorQuantity::Srgb).unwrap(),
            rgb[0]
        );
        ...
        assert_eq!(
            color_of_xyz(&xyz, &white, ColorQuantity::Luv).unwrap(),
            luv[0]
        );
        ...
        for (q, r) in [
            ...
            (
                ColorQuantity::Luv,
                ColorReference::Triple([50.0, 10.0, -20.0]),
            ),
    ...
    fn din99_matches_delta_path_and_white_yellow_hand() {
        ...
        assert_eq!(
            color_of_xyz(&xyz, &white, ColorQuantity::Din99).unwrap(),
            d99[0]
        );
        ...
        assert_eq!(
            w,
            100.0 * xyz[1]
                + 800.0 * (white[0] / ws - xyz[0] / s)
                + 1700.0 * (white[1] / ws - xyz[1] / s)
        );
        assert_eq!(
            tw,
            1000.0 * (white[0] / ws - xyz[0] / s) - 650.0 * (white[1] / ws - xyz[1] / s)
        );
        assert_eq!(
            e313_yellowness(&xyz, 1.3013, 1.1498),
            100.0 * (1.3013 * xyz[0] - 1.1498 * xyz[2]) / xyz[1]
        );
        ...
        for d in [
            idx_demand(
                ColorQuantity::Din99,
                ColorReference::Triple([50.0, 5.0, 5.0]),
            ),
    ...
    fn index_gates_name_all_sides() {
        let bad_q = |q, r: ColorReference, d: ColorDistance| {
            let (cmf_wl, cmf, illum_wl, illuminant) = toy();
            ColorDemand::new(
                0, cmf, cmf_wl, illuminant, illum_wl, q, r, d, 1.0, 1.3013, 1.1498, None,
            )
        };
        assert!(
            bad_q(
