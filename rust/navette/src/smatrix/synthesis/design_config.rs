...
fn as_bool(v: &Value, key: &str) -> Result<bool, String> {
    v.as_bool()
        .ok_or_else(|| format!("flag {key:?} must be a boolean"))
}

fn as_f64(v: &Value, key: &str) -> Result<f64, String> {
    v.as_f64()
        .ok_or_else(|| format!("flag {key:?} must be a number"))
}
...
pub(crate) fn apply_flag_map(
    ...
    for (k, v) in map {
        match k.as_str() {
            ...
            "rough_type" => {
                layer.rough_type = RoughnessType::try_from_i32(as_i32(v, k)?)
...
pub(crate) fn build_group(row: &GroupRow) -> Result<Group, String> {
    ...
    if row.optimization_mask.len() != 7 {
        return Err(format!(
            "group {:?}: optimization_mask needs 7 binary entries",
            row.name
        ));
    }
    ...
    for (i, v) in row.optimization_mask.iter().enumerate() {
        if *v != 0 && *v != 1 {
            return Err(format!(
                "group {:?}: optimization_mask must be binary",
                row.name
            ));
        }
        g.optimization_mask[i] = *v;
    }
    g.thickness_error_type = ErrorType::try_from_i32(row.thickness_error_type)
        .map_err(|e| format!("group {:?}: {e}", row.name))?;
    g.n_error_type = ErrorType::try_from_i32(row.n_error_type)
        .map_err(|e| format!("group {:?}: {e}", row.name))?;
    g.k_error_type = ErrorType::try_from_i32(row.k_error_type)
        .map_err(|e| format!("group {:?}: {e}", row.name))?;
...
pub(crate) fn eval_material(
    ...
    let model = if def.model == "TableMaterial" {
        "Table"
    } else {
        def.model.as_str()
    };
    ...
    if model == "Table" {
        let n = def
            .n_data
            .as_ref()
            .ok_or_else(|| "TableMaterial requires n_data".to_string())?;
        params.insert(
            "n_data".to_string(),
            Value::Array(vec![
...
pub fn build_design(
    ...
    let (stack, warnings) = DesignStack::from_design(
        amb,
        sub,
        &films,
        &nk_table,
