...
impl MaterialProvider for DictProvider {
    fn nk(&self, name: &str, wavelengths: &[f64]) -> Result<Vec<Complex64>, String> {
        let entry = self
            .entries
            .get(name)
            .ok_or_else(|| format!("DictProvider: unknown material '{name}'."))?;
        match &self.grid {
...
impl MaterialProvider for SpecProvider {
    fn nk(&self, name: &str, wavelengths: &[f64]) -> Result<Vec<Complex64>, String> {
        ...
        let entry = self
            .entries
            .get(name)
            .ok_or_else(|| format!("SpecProvider: unknown material '{name}'."))?;
        let nk = match entry {
            Entry::Array(v) => {
...
mod tests {
    ...
    fn shelf() -> (DictProvider, Vec<f64>) {
        ...
        entries.insert(
            "TiO2".to_string(),
            Entry::Array(vec![Complex64::new(2.35, 0.01); 3]),
        );
        entries.insert(
            "glass".to_string(),
            Entry::Array(vec![Complex64::new(1.52, 0.0); 3]),
        );
        (
            DictProvider::with_grid(entries, grid.clone()).unwrap(),
            grid,
        )
    }
    ...
    fn serve_contains_grid_match_python() {
        ...
        assert_eq!(
            p.nk("TiO2", &grid).unwrap(),
            vec![Complex64::new(2.35, 0.01); 3]
        );
    ...
    fn construction_and_insert_length_checked() {
        ...
        bad.insert(
            "x".to_string(),
            Entry::Array(vec![Complex64::new(1.0, 0.0); 5]),
        );
        assert!(DictProvider::with_grid(bad, vec![1.0, 2.0]).is_err());
        let (mut p, grid) = shelf();
        assert!(
            p.insert_array("new", vec![Complex64::new(1.0, 0.0); 2])
                .is_err()
        );
        p.insert_array("new", vec![Complex64::new(1.0, 0.0); 3])
            .unwrap();
    ...
    fn refresh_swaps_atomically() {
        ...
        entries.insert(
            "Si".to_string(),
            Entry::Array(vec![Complex64::new(3.5, 0.0); 2]),
        );
