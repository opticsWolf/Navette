    ...
    pub fn evaluate(&self, wavelengths: &[f64]) -> Result<Vec<Complex64>, String> {
        ...
        let out: Array1<Complex64> = match m {
            ...
            "Sellmeier" => {
                ...
                let out = crate::materials::sellmeier::sellmeier_nk(
            ...
            "SellmeierUrbach" => {
                let v = req_float(
                    ...
                    m,
                    &[
                        "B1", "C1", "B2", "C2", "B3", "C3", "alpha0", "Eu", "lambda_g",
                    ],
                )?;
            ...
            "Lorentz" => {
                ...
                crate::materials::lorentz::lorentz_nk(
                    w,
                    osc.view(),
            ...
            "Bruggeman" | "MaxwellGarnett" | "Looyenga" | "Lichtenecker" | "MoriTanaka"
            | "PowerLaw" => {
                ...
                let eps = match m {
                    ...
                    _ => crate::materials::ema::general_power_law(
                        hi,
                        hh,
                        f,
                        opt_float(p, "alpha", 0.5),
                    ),
                };
            ...
            other => {
                return Err(format!(
                    "Unknown material model {other:?}. Available: {MODELS:?}"
                ));
            }
        };
        Ok(out.to_vec())
...
fn req_osc(
    ...
    for r in rows {
        ...
        if row.len() != width {
            return Err(format!(
                "{what} needs an (N, {width}) array, got a row of width {}",
                row.len()
            ));
        }
        flat.extend(row);
...
fn req_spec(p: &BTreeMap<String, Value>, what: &str) -> Result<MaterialSpec, String> {
    let v = p
        .get(what)
        .ok_or_else(|| format!("Composite material missing '{what}' spec"))?;
    if v.is_null() {
...
mod tests {
    ...
    fn all_models_match_python() {
        ...
        let cases: &[(&str, Value, [[f64; 2]; 3])] = &[
            (
                "Konstant",
                serde_json::json!({"n": 2.0, "k": 0.1}),
                [[2.0, 0.1], [2.0, 0.1], [2.0, 0.1]],
            ),
            (
                "Table",
                serde_json::json!({"n_data": table(vec![300., 500., 800., 1200.], vec![2.6, 2.5, 2.4, 2.35]),
        "k_data": table(vec![300., 500., 800., 1200.], vec![0.02, 0.01, 0.005, 0.002])}),
                [[2.55, 0.015], [2.4, 0.005], [2.35, 0.002]],
            ),
            (
                "Cauchy",
                serde_json::json!({"A": 1.5, "B": 0.01, "C": 0.0001}),
                [
                    [1.56640625, 0.0],
                    [1.515869140625, 0.0],
                    [1.503921508789062, 0.0],
                ],
            ),
            (
                "CauchyUrbach",
                serde_json::json!({"A": 2.5, "B": 0.02, "C": 0.0005, "alpha0": 1e4, "Eu": 0.05, "lambda_g": 400.0}),
                [
                    [2.64453125, 0.0],
                    [2.532470703125, 2.200223592879712e-17],
                    [2.507888793945312, 8.180694225817397e-24],
                ],
            ),
            (
                "Sellmeier",
                serde_json::json!({"B1": 1.0396, "C1": 0.0060, "B2": 0.2318, "C2": 0.0200, "B3": 1.0105, "C3": 103.56}),
                [
                    [1.530834591147765, 0.0],
                    [1.510772050857463, 0.0],
                    [1.500018269952839, 0.0],
                ],
            ),
            (
                "SellmeierUrbach",
                serde_json::json!({"B1": 1.0396, "C1": 0.0060, "B2": 0.2318, "C2": 0.0200, "B3": 1.0105, "C3": 103.56, "alpha0": 1e4, "Eu": 0.05, "lambda_g": 400.0}),
                [
                    [1.530834591147765, 0.0],
                    [1.510772050857463, 2.200223592879712e-17],
                    [1.500018269952839, 8.180694225817397e-24],
                ],
            ),
            (
                "Lorentz",
                serde_json::json!({"osc": [[3.0, 0.2, 0.5], [4.5, 0.1, 0.7]], "epsilon_inf": 1.0}),
                [
                    [1.153731208791381, 1.621435145900531],
                    [1.573151551249795, 1.2352353127448e-02],
                    [1.502318258686829, 4.235632045690505e-03],
                ],
            ),
            (
                "Drude",
                serde_json::json!({"omega_p": 2.5, "gamma": 0.3, "epsilon_inf": 3.5}),
                [
                    [1.68992544212249, 1.845590373858986e-02],
                    [1.023761253547352, 2.371197698163733e-01],
                    [7.118613045903379e-01, 2.461407418588736],
                ],
            ),
            (
                "DrudeLorentz",
                serde_json::json!({"omega_p": 2.5, "gamma": 0.3, "epsilon_inf": 3.5, "osc": [[3.0, 0.2, 0.5]]}),
                [
                    [1.239022421780916, 1.519350784807504],
                    [1.308238569034786, 1.977760652565055e-01],
                    [7.444845390678393e-01, 2.360185402641237],
                ],
            ),
            (
                "CodyLorentz",
                serde_json::json!({"Eg": 1.5, "Et": 0.5, "Eu": 0.2, "osc": [[2.0, 0.5, 0.3, 25.0]], "epsilon_inf": 1.0}),
                [
                    [9.999931254129154e-01, 2.933688920744446e-05],
                    [1.00007490300154, 1.665040163139109e-07],
                    [1.000046511480627, 0.0],
                ],
            ),
            (
                "ForouhiBloomerSingle",
                serde_json::json!({"ib": [[3.0, 0.1, 6.0, 12.0]], "n_inf": 1.5}),
                [
                    [1.505731739947586, 3.296148880115328e-04],
                    [1.450778367057924, 0.0],
                    [1.451528702750885, 0.0],
                ],
            ),
            (
                "ForouhiBloomerMulti",
                serde_json::json!({"ib": [[3.0, 0.1, 6.0, 12.0], [4.0, 0.2, 5.0, 10.0]], "n_inf": 1.5}),
                [
                    [1.245603303142273, 3.296148880115328e-04],
                    [1.169425681065667, 0.0],
                    [1.23904677648164, 0.0],
                ],
            ),
            (
                "ForouhiBloomerMetal",
                serde_json::json!({"ib": [[3.0, 0.1, 6.0, 12.0]], "fe": [8.0, 0.1, 0.3], "n_inf": 1.5}),
                [
                    [2.926028938849746, 8.008632112202532],
                    [4.170044483376079, 7.544483643532266],
                    [5.792892498291914, 5.837037234100818],
                ],
            ),
            (
                "ForouhiBloomerMetal2021",
                serde_json::json!({"ib": [[3.0, 0.1, 6.0, 12.0]], "A_fe": 8.0, "B_fe": 0.1, "C_fe": 0.3, "n_inf": 1.5}),
                [
                    [2.926028938849746, 8.008632112202532],
                    [4.170044483376079, 7.544483643532266],
                    [5.792892498291914, 5.837037234100818],
            ...
            (
                "TaucLorentz",
                serde_json::json!({"Eg": 2.0, "osc": [[30.0, 3.0, 1.0]], "epsilon_inf": 1.0}),
                [
                    [1.780613483519783, 9.881532990951435e-01],
                    [1.495882707729832, 0.0],
                    [1.422104113544718, 0.0],
                ],
            ),
            (
                "UBF",
                serde_json::json!({"osc": [{"Eg": 2.0, "Ec": 3.0, "Eu": 0.5, "A": 20.0, "Gamma": 1.0, "gamma": 0.5}], "epsilon_inf": 1.0}),
                [
                    [2.096898993413011, 2.176879958480976],
                    [2.248023215134438, 1.696526148462792e-01],
                    [2.054295667079059, 5.904283218991443e-02],
                ],
            ),
            (
                "Bruggeman",
                serde_json::json!({"host": konst(1.5, 0.0), "inclusion": konst(2.0, 0.1), "fraction": 0.3}),
            ...
            (
                "Roughness",
                serde_json::json!({"bottom": konst(1.5, 0.0), "top": konst(2.0, 0.1)}),
    ...
    fn dispatch_errors_match_python() {
        ...
        assert!(
            spec("Konstant", serde_json::json!({"n": 1.5}))
                .evaluate(&[])
                .is_err()
        );
      ...
      .contains("linear-only"));
        assert!(
            bad("Table", serde_json::json!({"n_data": [[[400.0]], [[2.0]]]}))
                .contains("needs a (wavelengths, values) pair")
        );
        assert!(
            bad(
                "ForouhiBloomerMetal",
                serde_json::json!({"ib": [[1.0, 2.0, 3.0, 4.0]]})
            )
            .contains("needs fe")
        );
        assert!(
            bad(
                "Bruggeman",
                serde_json::json!({"host": konst(1.5, 0.0), "fraction": 0.5})
            )
