...
impl Structure {
  ...
  pub fn new(layers: Vec<Layer>, groups: HashMap<String, Group>) -> Self {
    Self {
      layers,
      groups: groups.into_iter().map(|(k, g)| (k, crate::structure::shared_group(g))).collect(),
    }
  }

  /// Share the caller's group handles (binding path: bakes stay visible).
  pub fn with_shared(layers: Vec<Layer>, groups: HashMap<String, crate::structure::SharedGroup>) -> Self {
    Self { layers, groups }
  }
    ...
    self
      ...
      .filter(|(_, layer)| {
        ...
        match self.groups.get(layer.material.as_str()) {
          Some(g) => g.borrow().optimization_mask[crate::structure::enums::OptMask::Thickness as usize] != 0,
          None => true,
        }
      })
      .map(|(i, _)| i)
  ...
  pub fn from_state(value: &Value) -> Result<Self, String> {
    ...
    if let Some(Value::Object(map)) = value.get("groups") {
      for (name, gs) in map {
        groups.insert(name.clone(), crate::structure::shared_group(Group::from_state(gs)?));
      }
    }
...
mod tests {
  ...
  fn helpers_behave() {
    ...
    let mut groups = HashMap::new();
    groups.insert("Si".to_string(), crate::structure::shared_group(Group::new("Si")));
    st.groups = groups;
