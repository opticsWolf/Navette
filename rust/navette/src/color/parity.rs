...
fn ucs1960_ucs() {
    ...
    assert_rows2(&uv, &golden::F07_XYZ_TO_UCS_UV, "ucs1960_xyz_to_ucs_uv");

    let mut xy = vec![[0.0; 2]; golden::F07_UVP_IN.len()];
    uv1976_to_xy(&golden::F07_UVP_IN, &mut xy);
    assert_rows2(&xy, &golden::F07_UV1976_TO_XY, "ucs1960_uv1976_to_xy");
    uv1960_to_xy(&golden::F07_UVP_IN, &mut xy);
    assert_rows2(&xy, &golden::F07_UV1960_TO_XY, "ucs1960_uv1960_to_xy");
}
